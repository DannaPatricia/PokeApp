...
<template>
  <div class="pagination">
    <router-link v-if="page > 1" :to="{ name: 'PokeView', query: { page: props.page - 1 } }">⬅
    </router-link>
    <span>Page {{ page }}</span>
    <router-link v-if="!isLastPage" :to="{ name: 'PokeView', query: { page: props.page + 1 } }">
      ➡
    </router-link>
  </div>
  <div class="pokemon-card-container">
    <!-- se realiza por cada itereacion un enlace hacia los detalles de ese pokemon -->
    <RouterLink :to="{ name: 'PokeViewDetails', params: { pokemonName: pokemon.name } }" v-for="pokemon in pokemonList"
      :key="pokemon.name" class="poke-card">
      <p>{{ pokemon.name }}</p>
      ...
      <img v-if="pokemon.sprites" :src="pokemon.sprites" alt="{{ pokemon.name }}" />
    </RouterLink>
  </div>
</template>
